...
export default function TasksList({ list }: ITaskListProps) {
	const { lists, setLists } = useLists();
	const { totalTasks, activeTasks, completedTasks } = useTasksCount(
		list.tasks
	);
			...
			if (temporaryList.id === list.id) {
				...
				for (const temporaryTask of temporaryList.tasks) {
					if (temporaryTask.id === taskId) {
						...
						temporaryTask.completedAt = temporaryTask.isDone
							? now
							: '';
					}
				}
